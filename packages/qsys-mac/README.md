...
> [`reowens/qsys-tools`](https://github.com/reowens/qsys-tools) (`packages/qsys-mac`),
...
same result by **dropping their installer onto a window**. You download `Install Q-SYS
Designer.dmg` and run the installer; it shows a setup UI that runs the recipe (via
...
`scripts/package.sh` produces a Gatekeeper-clean **`Install Q-SYS Designer.dmg`** holding two bundles:

- **Install Q-SYS Designer.app** — what you download + run. Deep-signed with a **Developer ID
...
```sh
RES="/Volumes/Install Q-SYS Designer/Install Q-SYS Designer.app/Contents/Resources"
...
Robert Owens. Source is public at <https://github.com/reowens/qsys-tools> (`packages/qsys-mac`).
